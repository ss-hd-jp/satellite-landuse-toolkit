...
**状態：** v1.0.0（2026-09-13 タグ付け、Zenodo でアーカイブ。DOI は Release ページと `CITATION.cff`）。
タグ付けまでに外部レビューを6回受けています。
初回レビューで初期ドラフトの不具合（複数タイル、期間終端、タイル命名、Sentinel-2の
位置合わせとオフセット、AOIマスク）が見つかり、修正しました。2回目でその修正を
確認したうえで、追加の課題——Sentinel-2の補正状態を判定できない時の処理、AOIを
切り落とす解析窓、ホットスポットの順位、シーンタグの再利用、被覆率分母のメモリ、
水域のないゾーンがCSVから消える、検出力のないテスト——が指摘され、対応しました。
3回目でそれらの解消を確認したうえで残った2点（一部バンドだけ要求した時のタグ再利用、
変化ゼロで再実行した時に残るクラスターCSV）を修正。4回目でその解消と実行記録の
バイト単位の再現を確認したうえで、もう1点（タグの前方一致により `early` の上書きが
`early_wet` を消しうる）が見つかり修正。5回目でその解消を確認したうえで、Windows固有の
1点（大小文字だけ違うタグは同じファイルになる）が見つかり修正。6回目でその解消を確認し、
確認範囲（差分・回帰テスト・タグ処理）では追加指摘なしでした。その後、国内の湿原での
実行で、同じタイル名のJRC製品2つの間でウィンドウが1画素ずれる不具合が見つかり、テスト付きで
修正しています。いずれのレビューも、特定地域での分類精度や、Sentinel-2の実シーン2時期解析を
検証したものではありません。26本の回帰テストが通過し（何を確認しているかは `tests/test_regression.py` に列挙。各レビューで追加した
テストは、そのレビューが見たコミットでは失敗します）、同梱の例示AOIでの実データ実行を
入力・版・コマンド・出力まで紐づけて [examples/run_record_example.md](examples/run_record_example.md)
に記録しています。対応範囲は「ほぼ全球」であって「任意の地域」ではありません
（*制限事項* 参照）。
...
04.00以降でフラグなし、`false` でベースライン不明）——製品を確認したうえで `--offset-a` /
`--offset-b` を指定してください。提供元フラグがあればフラグに従い、それと食い違う
`raster:bands` のoffsetは適用せずマニフェストに根拠として記録します。続いてSCL水域画素で検算し、50画素以上の水域の
...
- シーンタグはファイル名になります（`<tag>_B04.tif`、`<tag>_stac.json`）。バンド名にアンダースコアは
  なく、タグは1シーンIDに固定され、大小文字だけ違うタグは拒否します（Windows/macOSでは同じファイルになるため）。
