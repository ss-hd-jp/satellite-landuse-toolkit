...
確認範囲（差分・回帰テスト・タグ処理）では追加指摘なしでした。いずれのレビューも、
特定地域での分類精度や、Sentinel-2の実シーン2時期解析を検証したものではありません。
25本の回帰テストが通過し（何を確認しているかは `tests/test_regression.py` に列挙。各レビューで追加した
