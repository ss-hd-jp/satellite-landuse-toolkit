...
included), `forest_loss_summary.csv` (with `tiles_coverage_pct` — well below
99 % usually means a tile is missing; a very small zone can lose a fraction of
a percent to grid effects alone), `forest_loss_threshold_sensitivity.csv`,
`forest_loss_hotspots_from2016.csv` (ranked by loss area).
...
→ `water_transitions.csv` per zone (all ten classes plus a TOTAL row for every
zone, zeros included, with `tiles_coverage_pct`); `lake_water_summary.csv` for
the **connected area of occurrence ≥ 5 % containing the seed** (single tile
only). That region can include channels joined to the lake and is not a
validated lake boundary or the provider's Maximum Water Extent layer.
...
The search is a point query on the AOI centre; `change` measures how much of
the AOI lies inside both scenes (`in_both_scenes_pct`) and stops below
`--min-coverage`. Download with the helper so the STAC sidecar is kept —
`change` needs it to decide reflectance scaling. A tag is bound to one scene:
re-using `early` for a different scene stops unless you pass `overwrite=True`.
...
`stride`×10 m (default 20 m). The scaling decision per scene and its
water-pixel check are printed and written to the manifest. The run stops when
the correction state cannot be confirmed from the sidecar — then check the
product and state it per scene, e.g. `--offset-a none --offset-b applied` —
and when the water check is implausible (that asks for verification; it does
not tell you which correction is right). Read `in_both_scenes_pct`, then
`common_valid_pct`. A single-date NDVI difference also tracks crop phenology —
before calling anything permanent, repeat with a second season.
